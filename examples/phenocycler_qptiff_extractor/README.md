...
- Edit and modify `process.py` file:
  - Locate the line `config["input_directory"] = "./input"` and change the path value with your desired input directory. This directory should contain the qptiff file. 
  - Locate the line `config["output_directory"] = "./output"` and change the path value with your desired output directory. This directory will contain the resulting marker images. OBS: don't choose an output folder inside the stated input folder!
...
Phenocycler qptiff extractor generates in the chosen output_directory one image file per marker channel found in the qptiff file:
- `[filename]_[marker_name][extension]`: the extracted marker image, converted to the chosen bit depth and optionally normalised.
